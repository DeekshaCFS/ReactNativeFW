...
type TaskDetailsScreenProps = {
  ownerId: number;
  taskId: number;
  fallbackTask?: TaskListItem | null;
  customerName?: string;
...
const getTaskId = (item: TaskListItem) => Number(item.id ?? item.Id) || 0;
...
const getIssuedQty = (item: TaskItemDetail) => {
  const value = item.issuedQty ?? item.IssuedQty ?? item.itemQuantity ?? item.ItemQuantity;
  return value === undefined || value === null || value === '' ? NA : String(value);
};

const getUsedQty = (item: TaskItemDetail) => {
  const value = item.usedQty ?? item.UsedQty ?? item.usedItemQty ?? item.UsedItemQty;
  return value === undefined || value === null || value === '' ? NA : String(value);
};

const getItemName = (item: TaskItemDetail) =>
...
const TaskDetailsScreen = ({
  ...
  const [task, setTask] = useState<TaskListItem | null>(fallbackTask ?? null);
  ...
  useEffect(() => {
    ...
    const fetchTaskDetails = async () => {
      ...
      setError('');
      try {
        const response = await touchlessApi.getTaskDetails<TaskDetailsResponse>({
          userId: ownerId,
          taskId,
        });
        const result = response.resultData ?? response.ResultData ?? null;
  ...
  const itemDetails =
    (record.itemDetails as TaskItemDetail[] | undefined) ??
    (record.ItemDetails as TaskItemDetail[] | undefined) ??
    (record.multipleItemAssigned as TaskItemDetail[] | undefined) ??
    (record.MultipleItemAssigned as TaskItemDetail[] | undefined) ??
    [];
