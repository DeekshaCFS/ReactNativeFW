...
import { touchlessApi } from '../api/Api';
import type { RootStackParamList } from './VerifyOTPScreen';

type AMCDetailsScreenProps = NativeStackScreenProps<RootStackParamList, 'AMCDetails'>;
...
const AMCDetailsScreen: React.FC<AMCDetailsScreenProps> = ({ route, navigation }) => {
  ...
  const fetchAMCDetails = async () => {
    ...
    try {
      ...
      const response = await touchlessApi.getAMCReportDetails<any>({
        ownerId,
        amcsId,
        amcServiceDetailsId,
      });
      ...
      if (response?.resultData) {
        ...
        console.log('[AMC Details] Using resultData');
      } else if (response?.ResultData) {
        data = response.ResultData;
        console.log('[AMC Details] Using ResultData');
      } else if (response?.data) {
        data = response.data;
        console.log('[AMC Details] Using data property');
      } else if (Array.isArray(response)) {
        data = response[0];
        console.log('[AMC Details] Response is array, using first item');
      } else if (response && typeof response === 'object' && Object.keys(response).length > 0) {
        // Check if response has expected AMC fields
        const hasAMCFields = Object.keys(response).some(key => 
          key.toLowerCase().includes('amc') || 
          key.toLowerCase().includes('customer') || 
          key.toLowerCase().includes('product')
        );
        if (hasAMCFields) {
          data = response;
          console.log('[AMC Details] Using response directly (has AMC fields)');
        }
      }
  ...
  const getStringValue = (item: Record<string, unknown>, keys: string[]): string => {
  ...
  const extractResponseData = (response: unknown): Record<string, unknown> | null => {
    ...
    for (const candidate of candidates) {
      const data = Array.isArray(candidate) ? candidate[0] : candidate;
      if (data && typeof data === 'object' && Object.keys(data).length > 0) {
  ...
  const openEditModal = async () => {
    ...
    setEditLoading(true);
    try {
      const response = await touchlessApi.getAMCDetailsForEdit<any>({
        userId,
        amcsId: editAmcsId,
      });
      const fetchedData = extractResponseData(response) || (amcData as Record<string, unknown>);
  ...
  const saveAMCChanges = async () => {
    ...
    try {
      ...
      const response = await touchlessApi.putAMC(payload);
