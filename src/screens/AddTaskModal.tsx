...
import {
  type AddTaskDetailsRequest,
  type AdvanceServiceItem,
  type AdvanceServiceResponse,
  type CityItem,
  ...
  type FSRBindItem,
  type FSRBindListResponse,
  type ItemInventoryListItem,
  type MultipleItemAssignedRequest,
  type QuoteBindItem,
  type QuoteBindListResponse,
  type StateItem,
...
const normalizeItemOption = (item: ItemInventoryListItem): ItemOption => {
  const record = item as Record<string, unknown>;
  return {
...
const normalizeQuoteOption = (item: QuoteBindItem): QuoteOption => ({
...
const normalizeFSROption = (item: FSRBindItem): FSROption => ({
  id: getNumberField(item as Record<string, unknown>, [
    'fsrId',
    'FSRId',
    'fsrNumber',
...
const AddTaskModal: React.FC<AddTaskModalProps> = ({
  ...
  const loadFieldworkers = useCallback(() => {
    ...
    touchlessApi
      .getExpenseTechnicianList({ownerId})
      .then(response => {
  ...
  const loadTaskTags = useCallback(() => {
    ...
    touchlessApi
      .getTaskTags<TaskTagResponse>({userId: ownerId})
      .then(response => {
  ...
  const loadServiceTypes = useCallback(() => {
    ...
    setServiceTypeError('');
    touchlessApi
      .getAdvanceServiceList<AdvanceServiceResponse>({
        userId: ownerId,
        searchParam: '',
      })
      .then(response => {
        const items = extractArray<AdvanceServiceItem>(response);
        setServiceTree(buildServiceTree(items));
      })
      .catch(error => {
        setServiceTree([]);
        const message =
  ...
  const loadQuoteList = useCallback(() => {
    ...
    setQuoteError('');
    touchlessApi
      .getQuoteBindList<QuoteBindListResponse>({
        userId: ownerId,
        searchParam: '',
      })
      .then(response => {
        const items = extractArray<QuoteBindItem>(response);
        setQuoteList(
  ...
  const loadFsrList = useCallback(() => {
    ...
    setFsrError('');
    touchlessApi
      .getFSRBindList<FSRBindListResponse>({
        userId: ownerId,
        searchParam: '',
      })
      .then(response => {
        const items = extractArray<FSRBindItem>(response);
        setFsrList(
  ...
  const loadItemOptions = useCallback(
    (searchParam: string) => {
      ...
      touchlessApi
        .getItemList({ownerId, searchParam})
        .then(response => {
  ...
  const handleAddTaskSubmit = async () => {
    ...
    const multipleItemAssigned: MultipleItemAssignedRequest[] = taskItemRows
    ...
    const payload: AddTaskDetailsRequest = {
    ...
    try {
      await touchlessApi.addTaskDetails(payload);
