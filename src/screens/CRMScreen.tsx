...
export const normalizeTaskTag = (item: TaskTag): TaskTagOption => ({
  ...
  label:
    String(
      item.taskTagName ?? item.TaskTagName ?? '',
    ).trim() || 'Task',
});
...
const CRMScreen = ({
  ...
  const handleAddEnquirySubmit = async () => {
    ...
    try {
      if (isEditing) {
        ...
        const updateResponse = await touchlessApi.updateEnquiry(updatePayload);
        ...
        await touchlessApi.addEnquiry(payload);
        closeAddEnquiryModal();
  ...
  const getEnquiryFormShareUrl = () =>
    `${URLConstant.Base.GET_LINK}${encryptUserId(ownerId)}`;
