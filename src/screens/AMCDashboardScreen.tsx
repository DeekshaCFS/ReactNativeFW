...
import {
  type AMCLookupItem,
  type AMCLookupResponse,
  type AMCListItem,
  type AMCListResponse,
  type AMCTypeItem,
  type AMCTypeResponse,
...
const normalizeAMCType = (item: AMCTypeItem): AMCTypeOption | null => {
  const id = Number(item.amcTypeId ?? item.AMCTypeId);
  const name = String(item.amcTypeName ?? item.AMCTypeName ?? '').trim();
...
const AMCDashboardScreen = ({
  ...
  const loadAMCTypes = useCallback(async () => {
    try {
      const response = await touchlessApi.getAMCTypes<AMCTypeResponse>();
  ...
  const loadAMCFormLookups = useCallback(async () => {
    try {
      const [occurrenceResponse, reminderResponse] = await Promise.all([
        touchlessApi.getAMCServiceOccurrenceTypes<AMCLookupResponse>(),
        touchlessApi.getAMCReminderModes<AMCLookupResponse>(),
      ]);
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={styles.toolbar}>
          ...
          <Text style={styles.toolbarTitle}>AMC</Text>
