...
import {
  ...
  type SaveQuotationItemRequest,
  type SaveQuotationServiceRequest,
  touchlessApi,
} from '../api/Api';
...
const getServiceTypeId = (item: LeadServiceTypeItem) =>
  Number(item.serviceTypeId ?? item.ServiceTypeId ?? item.serviceTypeID ?? item.ServiceTypeID ?? item.id ?? item.Id ?? 0);
...
const getItemId = (item: ItemInventoryListItem) =>
  getItemFieldNumber(item, ['itemId', 'ItemId', 'id', 'Id']);

const getItemName = (item: ItemInventoryListItem) =>
  getItemFieldString(item, ['name', 'Name', 'itemName', 'ItemName']);

const getItemPrice = (item: ItemInventoryListItem) =>
  getItemFieldNumber(item, ['salesPrice', 'SalesPrice']);
...
const AddQuoteModal = ({visible, ownerId, onClose, onSuccess}: AddQuoteModalProps) => {
  ...
  const loadServiceTypes = useCallback(async () => {
    ...
    try {
      const response = await touchlessApi.getLeadServiceTypeList({ownerId});
      setServiceTypes(response.resultData ?? response.ResultData ?? []);
    } catch (error) {
      setServiceTypes([]);
    } finally {
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      const response = await touchlessApi.getItemListV2({ownerId});
  ...
  const handleSave = async () => {
    ...
    try {
      await touchlessApi.saveQuotationNonOwner({
        UserId: ownerId,
        ...
        BuildingFlatNumber: buildingFlatNumber.trim(),
